...
前提:
  ...
  - group-settings/ 配下に病棟ディレクトリがあり、その下に YYYY-MM-DD/setting.lp
    ...
    ※このスクリプトでは group_settings_dir は互換のために引数として残しているが未使用

ポイント:
  - 病棟の「所属判定」は group_settings（timeline）側で行うのではなく、
    「その病棟の past-shifts から作る分布」として扱う（=病棟単位の実績分布比較）
  - 病棟名は表示用に短い英語ラベルに変換する（内部IDとして使用）
      例: 4階南病棟 -> 4S, 7階北病棟 -> 7N
  - プロット対象の病棟は CLI で指定できる（内部ラベルで指定）
      --plot-wards ICU,NICU,GCU,4S,7N,...
  - 描画の表示ラベルは --label-lang で切替
      --label-lang en (デフォルト) / --label-lang ja
...
def load_past_shifts_by_ward(
    ...
    戻り値:
      ...
      label_to_raw: {label: raw_name}  (表示/デバッグ用)
    """
    ...
    for f in list_past_shift_files(past_dir):
        ...
        if label in ward_to_seqs:
            print(
                f"# WARNING: duplicate label '{label}' from file: {f} (already exists). Overwrite."
            )
    ...
    if not ward_to_seqs:
        if plot_wards is None:
            ...
            raise SystemExit(
                f"No wards loaded. Check --plot-wards={sorted(plot_wards)} and mapping."
            )
...
def js_distance(c1: Counter, c2: Counter, alpha: float, support: str, n: int) -> float:
    ...
    Laplace smoothing with alpha, support:
      - observed_ab: V = |keys union|  (ペアごとに語彙が変わる)
...
def plot_heatmap(
    ...
    for i in range(R):
        for j in range(C):
            ...
            plt.text(
                j, i, f"{v:.3f}", ha="center", va="center", fontsize=12, color=txt_color
            )
...
def main():
    ...
    ap.add_argument(
        "past_shifts_dir", help="exp/2019-2025-data/past-shifts/ (or a single .lp)"
    )
    ap.add_argument(
        "group_settings_dir",
        help="exp/2019-2025-data/group-settings/ (not used; kept for compatibility)",
    )
    ...
    ap.add_argument(
        "--laplace-support", choices=["observed_ab", "all"], default="observed_ab"
    )
    ...
    ap.add_argument(
        ...
        choices=["en", "ja"],
        default="en",
        help="Axis label language: en=short labels (default), ja=raw Japanese ward names.",
    )

    # ★追加: 日本語描画時のフォント指定（未指定なら matplotlib のデフォルト）
    ap.add_argument(
        "--jp-font",
        default="",
        help='Font family for Japanese labels (e.g. "IPAexGothic", "Noto Sans CJK JP"). Used only when --label-lang ja.',
    )

    args = ap.parse_args()

    # 日本語表示時にフォントを指定したい場合
    if args.label_lang == "ja" and args.jp_font.strip():
        plt.rcParams["font.family"] = args.jp_font.strip()
    ...
    # 病棟ごとに past-shifts を読む（内部ラベルに変換）
    ...
    # 表示ラベル切替
    if args.label_lang == "ja":
        # raw が取れない場合（RAW:xxx など）はフォールバック
        labels = [f"{label_to_raw.get(w, w)}({totals[w]})" for w in wards]
    else:
    ...
    for n in range(args.nmin, args.nmax + 1):
        ...
        for wi in wards:
            ...
            for wj in wards:
                row.append(
                    js_distance(
                        counters[wi], counters[wj], args.alpha, args.laplace_support, n
                    )
                )
            mat.append(row)

        out = os.path.join(
            args.outdir, f"heatmap_ward_x_ward_{n}gram_{args.label_lang}.png"
        )
        title = f"P(gram), n={n}"
