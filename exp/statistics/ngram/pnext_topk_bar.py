...
def select_basis_events(
    eventA: Counter, eventB: Counter, topk: int, basis: str
) -> List[EventKey]:
    if basis == "A":
...
def print_compare_table(
    ...
    for i, (prefix, nxt) in enumerate(keys, 1):
        ...
        # skip のとき nan が混じるので表示上 0 にしない（nanのまま）
...
def plot_compare_bar(
    ...
    for k, lab in zip(keys, labels):
        ...
        valsA.append(0.0 if pa != pa else pa)  # nan -> 0 for safety
