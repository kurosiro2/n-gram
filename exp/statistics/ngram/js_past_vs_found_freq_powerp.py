...
変更点（今回）:
  - n=1 のデータ数(T)は表示しない（計算もしない）
  - AUTO(found)行のセル注釈:
        1行目: foundの値 (小数3桁)
        2行目: MANUALのIQR(Q1–Q3)からの “はみ出し差” を符号つきで表示 (diff)
            inside IQR -> (+0.000)
            above Q3    -> (+(found - Q3))
            below Q1    -> (-(Q1 - found))
      例: 0.321\n(+0.045)
...
NOTE:
  - DejaVu Sans で欠けやすい「全角チルダ(～)」「全角縦棒(｜)」を
    ASCII の "~" "|" に置換して警告を抑止。
"""
...
# -------------------------------------------------------------
# matplotlib: global font / layout
#   ※セル内文字サイズは plot関数側で個別制御
# -------------------------------------------------------------
plt.rcParams.update({
    "font.size": 16,
    "axes.titlesize": 18,
...
def plot_heatmap_period_x_n(
    ...
    # ★同じサイズ感 + diffを大きく + manualのバーは小さめ
    VALUE_FS = 19   # foundの値 / fullの数値
    MANUAL_FS = 19  # summary manualのQ1/Q3
    BAR_FS = 12     # summary manualの2行目「|」
    DIFF_FS = 16    # ★(+/-) をもっと大きく
    ...
    for i in range(R):
        for j in range(C):
            ...
            if "\n" in s:
                ...
                # fallback: 2行だけど foundじゃない → manual扱い
                if len(parts) == 2:
                    top, bottom = parts
                    plt.text(j, i - 0.18, top, ha="center", va="center",
                             fontsize=MANUAL_FS, color=txt_color)
...
def main():
    ...
    def apply_past_mode_and_annotations(
        ...
        # MANUAL IQR (全past半期から列ごとに計算) は、found注釈に使う
        q1s, meds, q3s = manual_iqr_by_col(past_mat)
        ...
        if args.past_mode == "full":
            ...
            return mat, ann, labels

        # summary: pastを1行へ（注釈は "Q1~\n|\nQ3" で作る）
        past_label = f"MANUAL"
        ...
        return mat2, ann2, labels2

    # Heads
    if not args.only_nonheads:
        mat_h = build_matrix(is_heads=True)
        mat_h2, ann_h2, labels_h2 = apply_past_mode_and_annotations(mat_h)
    ...
    mat_n2, ann_n2, labels_n2 = apply_past_mode_and_annotations(mat_n)
    ...
    plot_heatmap_period_x_n(
        out_n,
        title=f"JSDistance(AUTO vs {base_key} MANUAL)",
