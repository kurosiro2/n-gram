...
from ngram_past_shifts_group import ngram_counts_by_group  # ★ これを使って "All" のスコアを合わせる
...
def main():
    if len(sys.argv) < 3:
        print("python 2gram_topK_pca.py [shift_file] [setting_path]")
        sys.exit(1)
    ...
    # ---------- 1.5) Group=\"All\" の 2-gram スコアを ngram スクリプトと同じ方法で計算 ----------
    # n=1,2 のカウンタを ngram_past_shifts_group と同じロジックで作る
    counters_1 = ngram_counts_by_group(seqs, group_timeline, 1)
    counters_2 = ngram_counts_by_group(seqs, group_timeline, 2)

    uni_all = counters_1.get("All", Counter())
    bi_all  = counters_2.get("All", Counter())

    if not bi_all:
        print("ERROR: 'All' group bigram counter is empty.", file=sys.stderr)
        sys.exit(1)

    total_bi_all = sum(bi_all.values()) or 1

    all_scores = {}  # pair_str -> score
    for (s1, s2), c in bi_all.items():
        base = uni_all.get((s1,), 0)
        if base <= 0:
            continue
        prob       = c / base                 # P(next|prev)
        freq_share = c / total_bi_all         # bigram の相対度数
        score      = prob * freq_share        # ★ ngram_past_shifts_group と同じ式
        pair_str   = f"{s1}->{s2}"
        all_scores[pair_str] = score

    # score の大きい順に TopK を決める
    sorted_pairs = sorted(all_scores.items(), key=lambda kv: kv[1], reverse=True)
    topK_pairs = [p for p, _ in sorted_pairs[:TOP_K]]

    # ここだけ表示（μとして score_all を出す）
    print(f"\n===== Top{TOP_K} pairs:", file=sys.stderr)
    for p, v in sorted_pairs[:TOP_K]:
        print(f"  {p}: μ={v:.6f}", file=sys.stderr)
    ...
    # ---------- 3) 各セグメントについて 2-gram スコア計算（これは従来どおり） ----------
    ...
    # ---------- 6) （重要）PCA でもこの topK_pairs を使う ----------
    print(f"\n===== Using Group=All Top{TOP_K} for PCA =====", file=sys.stderr)
    for p in topK_pairs:
        print("   ", p, file=sys.stderr)

    # ---------- 7) 各セグメントについて K 次元ベクトル生成 ----------
    X = []
    for uid in unit_ids:
    ...
    for g in all_groups:
        ...
        plt.xlabel("PC1")
        plt.ylabel("PC2")
        plt.title(f"Nurse Patterns PCA (Top{TOP_K} 2-grams, timeline segments) - {g}")
        plt.legend()
        plt.grid(True)
        ...
        fname = f"pca_top{TOP_K}_{sanitize(g)}.png"
        plt.savefig(fname, dpi=300)
        plt.close()

        print(f"# saved: {fname}", file=sys.stderr)

    # 10-2) ALL_by_group（複合グループを 1 カテゴリとして扱う）
    combined_labels = []
    for uid in unit_ids:
        gset = unit_to_groups.get(uid, {"Unknown"})
        if not gset:
    ...
    # 10-2-a) Unknown 含む ALL_by_group（凡例を外に出す）
    ...
    plt.xlabel("PC1")
    plt.ylabel("PC2")
    plt.title(f"Nurse Patterns PCA (Top{TOP_K} 2-grams, timeline segments)\nALL by GroupColor")
    plt.grid(True)
    plt.xlim(x_min, x_max)
    plt.ylim(y_min, y_max)
    plt.gca().set_aspect("equal", adjustable="box")
    ...
    fname_all = f"pca_top{TOP_K}_ALL_by_group.png"
    plt.savefig(fname_all, dpi=300)
    plt.close()
    print(f"# saved: {fname_all}", file=sys.stderr)

    # 10-2-b) Unknown 除外 ALL_by_group
    ...
    if unique_no_unknown:
        ...
        plt.ylabel("PC2")
        plt.title(
            f"Nurse Patterns PCA (Top{TOP_K} 2-grams, timeline segments)\n"
        ...
        fname_no_unknown = f"pca_top{TOP_K}_ALL_by_group_no_unknown.png"
