...
要点:
  ...
  - MLE と Laplace(add-k) を両方計算して同じ図に表示
...
def laplace_probs(next_counter: Counter, Ny: int, k: float) -> List[float]:
    """
    Laplace(add-k) 確率ベクトル:
      P_LAP(x|y) = (c(y,x)+k) / (c(y) + k|X|)
    Ny=0 でも定義できて、全て 1/|X| になる。
    """
    denom = Ny + k * X_SIZE
    if denom <= 0:
        # k>0前提なので基本来ないが保険
        return [1.0 / X_SIZE] * X_SIZE
    return [(next_counter.get(x, 0) + k) / denom for x in VALID_SHIFTS]


def print_debug(tag: str, prefix: Prefix, next_counter: Counter, Ny: int, k: float) -> None:
    print("")
    print("=" * 110)
    print(tag)
    print("=" * 110)
    print(f"prefix={','.join(prefix)}  Ny=c(prefix)={Ny}   |X|={X_SIZE}  laplace_k={k}")
    ...
    pmle = mle_probs(next_counter, Ny)
    plap = laplace_probs(next_counter, Ny, k)

    print(f"{'next':>4}  {'count':>6}  {'P_MLE':>10}  {'P_LAP':>10}")
    for x, m, l in zip(VALID_SHIFTS, pmle, plap):
...
def run_one_ward(
    ...
    # debug print
    print_debug(f'Ward="{ward_name}" Heads  {labelA}', target_prefix, hA, Ny_hA, laplace_k)
    print_debug(f'Ward="{ward_name}" Heads  {labelB}', target_prefix, hB, Ny_hB, laplace_k)
    print_debug(f'Ward="{ward_name}" NonHeads  {labelA}', target_prefix, nA, Ny_nA, laplace_k)
    print_debug(f'Ward="{ward_name}" NonHeads  {labelB}', target_prefix, nB, Ny_nB, laplace_k)

    # probs (MLE + Laplace)
    phA_mle = mle_probs(hA, Ny_hA)
    phB_mle = mle_probs(hB, Ny_hB)
    pnA_mle = mle_probs(nA, Ny_nA)
    pnB_mle = mle_probs(nB, Ny_nB)

    phA_lap = laplace_probs(hA, Ny_hA, laplace_k)
    phB_lap = laplace_probs(hB, Ny_hB, laplace_k)
    pnA_lap = laplace_probs(nA, Ny_nA, laplace_k)
    pnB_lap = laplace_probs(nB, Ny_nB, laplace_k)

    # titles include Ny
    note_h = f"(Ny A={Ny_hA}, Ny B={Ny_hB}, k={laplace_k})"
    note_n = f"(Ny A={Ny_nA}, Ny B={Ny_nB}, k={laplace_k})"

    safe_ward = ward_name.replace("/", "_")
    safe_prefix = prefix_str.replace(",", "-")

    out_h = os.path.join(outdir, f"compare_prefix-{safe_prefix}_heads_ward-{safe_ward}_N{n}_{labelA}_vs_{labelB}_withLaplace.png")
    out_n = os.path.join(outdir, f"compare_prefix-{safe_prefix}_nonheads_ward-{safe_ward}_N{n}_{labelA}_vs_{labelB}_withLaplace.png")
...
def main():
    ...
    if os.path.isdir(args.past_shifts):
        ...
        for fname in lp_files:
            ...
            print("\n" + "#" * 120)
            print(f'# RUN ward="{ward}" prefix="{args.prefix}" N={args.n}  A={args.a_start}-{args.a_end}  B={args.b_start}-{args.b_end}  k={args.laplace_k}')
            print("#" * 120)
